...
1. `admin.html`을 열고 작업 ID, 상·하의 코드, 상품명, 컬러만 입력합니다.
2. 상의 이미지 최대 5장과 하의 이미지 최대 5장을 업로드합니다. 참고 이미지 최대 3장과 생성 후보 최대 3장은 필요할 때 추가합니다.
3. 업로드 즉시 미리보기를 확인하고 각 이미지의 정면·디테일·후면·원단·후보 역할을 지정합니다.
4. `1. 상품 분석 만들기`를 누릅니다. 소재·디테일·기장·실루엣·보존/금지 요소는 자동으로 채워지며, 필요할 때만 `고급 설정 열기`에서 수정합니다.
5. `2. 모델컷 프롬프트 만들기`를 눌러 실제 상품 일치도 원칙이 포함된 프롬프트를 만들고 복사하거나 TXT로 내려받습니다.
6. `3. 작업 저장하기`를 눌러 현재 브라우저의 IndexedDB에 저장합니다. 성공 메시지와 오른쪽 저장 작업 카드가 즉시 표시됩니다.
7. `4. 검토판에서 확인하기`를 눌러 방금 저장한 작업을 `index.html` 검토판에서 확인합니다.
...
| 항목 | 결과 | 확인 내용 |
|---|---|---|
| 기본 입력 단순화 | 통과 | 첫 화면에는 작업 ID, 상·하의 코드·상품명·컬러 7개 입력만 표시되고 고급 설정은 닫힌 상태로 시작 |
| 단계 안내 | 통과 | 상품코드 입력부터 저장·검토판 확인까지 8개 행동 순서를 화면 상단에 표시 |
| 다중 이미지 미리보기 | 통과 | 상의 2장·하의 2장·참고 1장 업로드 직후 역할 선택이 포함된 미리보기 표시 |
| 후보 이미지 대기 | 통과 | 생성 후보를 올리지 않은 작업도 저장 가능하며 검토판에서 `이미지 연결 대기`로 표시 |
...
| 저장 피드백 | 통과 | `S941_N260007_SIMPLE` 저장 성공 문구와 오른쪽 저장 작업 카드가 즉시 표시됨 |
| 검토판 연결 | 통과 | 저장 완료 링크가 `index.html#card-S941_N260007_SIMPLE_candidate_01`로 이동하고 해당 카드 렌더링 확인 |
