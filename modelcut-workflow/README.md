...
1. `admin.html`을 열고 작업 ID와 상·하의 상품 정보를 입력합니다.
2. 상의 이미지 최대 5장, 하의 이미지 최대 5장, 참고 이미지 최대 3장, 생성 후보 최대 3장을 선택합니다.
3. 업로드 즉시 미리보기를 확인하고 각 이미지의 정면·디테일·후면·원단·후보 역할을 지정합니다.
4. 자동 생성된 상품 분석 요약을 확인하고 필요한 항목을 직접 수정합니다.
5. “프롬프트 자동 생성”을 눌러 실제 상품 일치도 원칙이 포함된 생성 프롬프트를 만듭니다.
6. “작업 저장”을 눌러 현재 브라우저의 IndexedDB에 저장합니다.
7. `index.html` 검토판을 열어 기존 샘플과 새 사용자 작업을 함께 확인합니다.
